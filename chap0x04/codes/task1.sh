...
# 对jpeg/png/jpg格式图片在保持原始宽高比的前提下压缩分辨率
...
function compressResolution {
    ...
    for i in *;do
        type=${i##*.}
        # 删除最后一个.及左边全部字符
        if [[ ${type} != "jpeg" && ${type} != "png" && ${type} != "jpg" ]]; then continue; fi;
...
# 对图片批量添加自定义文本水印
# convert filename1 -pointsize 16 -fill black -gravity center -draw "text 10,10 'watermark' " filename2 中心水印
# convert filename1 -pointsize 16 -fill black -gravity southeast -draw "text 10,10 'watermark' filename2 右下角水印
function watermark {
    for i in *;do
        type=${i##*.}
        # 删除最后一个.及左边全部字符
...
# 将png/jpg图片统一转换为jpg格式图片
# convert xxx.png xxx.jpg
function transform2Jpg {
    for i in *;do
        type=${i##*.}
        # 删除最后一个.及左边全部字符
        if [[ ${type} != "png" && ${type} != "jpg" ]]; then continue; fi;
        filename=${i%.*}".jpg"
        convert "${i}" "${filename}"
